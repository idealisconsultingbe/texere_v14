...
class LotReceptionForm(models.Model):
    ...
    state = fields.Selection(selection=[('confirmed', 'Confirmed'), ('sent', 'Sent'), ('approved', 'Approved'), ('closed', 'Closed')], tracking=True, index=True, readonly=True, string='Status', default='confirmed')
    ...
    def send_form(self):
        ...
        if self.env.context.get('send_form'):
            ...
            # TODO : reception form action may be used to notify users according to RR state
            # for point in checks.mapped('point_id'):
            #     point.message_post(
            #         subject=_('R&R form to Validate'),
            #         body=body,
